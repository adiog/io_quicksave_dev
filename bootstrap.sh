...
{
    MEMCACHED_COMPILATION_DIR=`BASH_MKTEMP_DIR`
    cd ${MEMCACHED_COMPILATION_DIR}
    git clone https://github.com/adiog/libmemcached
    cd libmemcached
...
pip_quicksave
qs_python
if ! grep -q touch /usr/include/libmemcached-1.0/memcached.hpp; then
    compile_memcached
fi
